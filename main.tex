...
% 它的預設內容是都關掉，可依需要打開
\input{scripts/utils/headerfooter}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage{tipa}     % JEFF
\usepackage{comment}  % JEFF

% 常見的 notation 指令
% 請修改 notations.tex 加入自己需要的 notations
% \input{scripts/utils/notations}

% Title Page
% 請修改 title_settings.tex 設定論文題目/指導教授等變數
\input{scripts/utils/title_settings}

% 載入中文名詞的定義：例如，Figure -->「圖」, Chapter -->「第 x 章」
\input{scripts/utils/ntu_definitions}

% 如果不需要以中文數字一、二、三呈現章別，例如「第一章」
% 則請把下列介於 >>> 與 <<< 之間
% 的文字行關掉 (行首加上百分號), 會以「第 1 章」呈現
%% 中文數字章別 >>>
\input{scripts/utils/ntu_chnum}
%% <<< 中文數字章別
...
% >>>
\input{scripts/before/chap_main}
